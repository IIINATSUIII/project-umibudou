...
export function validateQuestionnaireInput(input: unknown): QuestionnaireValidationResult {
  ...
  const booleanKeys = ['heartDisease','respiratoryDisease','earDisease','epilepsy','diabetes','pregnant','panicDisorder','medication','latexAllergy','alcoholLastNight','alcoholToday','flightWithin48h','hasCCard','agreeRisk','agreeMedical','agreePhoto'] as const
  ...
  return { ok: true, data: {
    ...
    gender: source.gender as QuestionnaireData['gender'], address, phone,
    emergencyName, emergencyRelation, emergencyPhone,
    heartDisease: values.heartDisease, respiratoryDisease: values.respiratoryDisease, earDisease: values.earDisease,
    ...
    condition: source.condition as QuestionnaireData['condition'], flightWithin48h: values.flightWithin48h,
