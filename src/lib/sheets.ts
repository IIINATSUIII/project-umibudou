...
export const HEADERS = {
  ...
  QUESTIONNAIRES: [
    ...
    'qrToken','qrIssuedAt','qrExpiresAt','qrUsed','doctorClearance','staffCheckStatus','staffCheckNote',
  ...
  ROSTER: [
    'id','diveDate','reservationId','questionnaireId','customerId','name','nameKana','birthDate','age','gender',
    'address','phone','emergencyContact','emergencyPhone','course','staffName','checkedInAt','checkInMethod',
  ],
  CUSTOMERS: [
...
export async function updateQuestionnaire(id: string, data: Partial<QuestionnaireData>): Promise<void> {
  ...
  await updateRowById(SHEET.QUESTIONNAIRES, HEADERS.QUESTIONNAIRES, id, { ...existing, ...data })
}
