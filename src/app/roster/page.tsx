...
const METHOD_LABELS: Record<string, string> = { qr: 'QR読取', manual: '手動照合' }
...
function toCsv(rows: RosterEntry[]): string {
  ...
  const lines = rows.map((r) => [
    ...
    METHOD_LABELS[r.receivedMethod] ?? r.receivedMethod,
  ].map(escape).join(','))
...
export default function RosterPage() {
  ...
  const courses = useMemo(
    () => Array.from(new Set(roster.map((r) => r.courseName).filter(Boolean))),
    [roster]
  )
  ...
  const filtered = roster
    ...
    .sort((a, b) => a.diveDate.localeCompare(b.diveDate) || a.receivedAt.localeCompare(b.receivedAt))
          ...
          {filtered.length === 0 ? (
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.map((r) => (
                  <tr key={r.id}>
                    <td className="px-3 py-2 whitespace-nowrap font-medium text-gray-800">{r.lastName} {r.firstName}</td>
                    <td className="px-3 py-2 whitespace-nowrap text-gray-500">{r.lastNameKana} {r.firstNameKana}</td>
                    <td className="px-3 py-2 whitespace-nowrap">{r.birthDate}</td>
                    <td className="px-3 py-2 whitespace-nowrap">{r.age}</td>
                    <td className="px-3 py-2 whitespace-nowrap">{GENDER_LABELS[r.gender] ?? r.gender}</td>
                    ...
                    <td className="px-3 py-2 whitespace-nowrap">{r.courseName}</td>
                    <td className="px-3 py-2 whitespace-nowrap">{r.staffName}</td>
                    <td className="px-3 py-2 whitespace-nowrap">{new Date(r.receivedAt).toLocaleString('ja-JP')}</td>
                    <td className="px-3 py-2 whitespace-nowrap">{METHOD_LABELS[r.receivedMethod] ?? r.receivedMethod}</td>
                  </tr>
                ))}
              </tbody>
            </table>
